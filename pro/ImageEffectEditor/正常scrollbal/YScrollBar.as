...
package
{
	...
	public class YScrollBar extends EventDispatcher {
		...
		private function checkObject(content:Sprite, scrollBar:Sprite)
		{
			_content = content;
		...
		private function _refresh(event : MouseEvent = null):void 
		{
			...
			if (_content[_width_height] > _area_width_height)
			{
				...
				_block[_width_height]=_allDistance*Math.pow(per,1/3);
		...
		private function poleDown(event : Event=null):void 
		{
			...
			if (_content[_width_height]>_area_width_height )
			{
				...
				_content[_x_y] += (newY - _content[_x_y]) / _ease;
			}
		}
		 ...
		 */
		public function init(content:Sprite, scrollBar:Sprite , areaX:Number, areaY:Number, areaWidth:Number, areaHeight:Number, scollType:String = YScrollBar.V, ease:Number = 0)
		{
